...
class FloatVectorChromosome(Chromosome[FloatPhenotype, FloatGenotype]):
    ...
    @staticmethod
    def phenotype_to_genotype(phenotype: FloatPhenotype, **kwargs) -> FloatGenotype:
    ...
    def genotypes(self, genes: List[FloatGenotype]) -> None:
        ...
        if len(genes) != vector_size:
            raise ValueError('Tried to set FloatParameterChromosome genotypes with wrong number of genes ({}). Expected {}.'
        ...
        for value in values:
            if value < lower_bound or value > upper_bound:
                raise ValueError('Tried to set FloatParameterChromosome genes with gene out of boundaries ({}). Expected [{}, {}].'
    ...
    def phenotypes(self) -> List[FloatPhenotype]:
        return [FloatVectorChromosome
            .genotype_to_phenotype(self.genotypes[i])
            for i in range(len(self.genotypes))]

    @phenotypes.setter
    def phenotypes(self, _phenotypes: List[FloatPhenotype]) -> None:
        self._genotypes = [FloatVectorChromosome
